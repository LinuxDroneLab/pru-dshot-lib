 ...
 */
#include <include/pru_dshot_driver.h>
#include <sys_pwmss.h>
...
uint8_t pru_dshot_lib_initialized[3] = { 0, 0, 0 };
PruDShotLibConfig* pru_dshot_lib_config = 0;
unsigned char pru_dshot_lib_cmd_rsp[4] = { 0 };
...
uint8_t pru_dshot_lib_IsConfigured(uint8_t pwmssDevice)
{
    return (pru_dshot_lib_config != 0);
}
uint8_t pru_dshot_lib_IsInitialized(uint8_t pwmssDevice)
{
    return pru_dshot_lib_initialized[pwmssDevice];
}
uint8_t pru_dshot_lib_IsRunning(uint8_t pwmssDevice)
{
    return (PRU_PWMSS[pwmssDevice]->CLKCONFIG_bit.EPWMCLK_EN);
}
uint8_t pru_dshot_lib_IsCmdSupported(unsigned char* cmd, uint8_t numBytes)
{
...
uint8_t pru_dshot_lib_Init(uint8_t pwmssDevice)
{
    /*
     * A T T E N Z I O N E!
    ...
    if (pru_dshot_lib_initialized[pwmssDevice] == 0)
    {
//        (*(CM_PER_PWMCSS_CLKCTRL[pwmssDevice])) = 2; /* enable module */
        ...
        PRU_PWMSS[pwmssDevice]->EPWM_CMPCTL = 0b0101; // LOADA/BMOD when CNT=PRD
        PRU_PWMSS[pwmssDevice]->EPWM_ETSEL = 0b1001; // INT abilitata e scatta quando CNT == 0
        ...
        *PWMSS_CTRL_REG = 7; // enable  pwmss_ctrl Register: bits pwmss2_tbclk, pwmss1_tbclke, pwmss0_tbclke
        pru_dshot_lib_initialized[pwmssDevice] = 1;
    }
    return 1;
}

uint8_t pru_dshot_lib_Start(uint8_t pwmssDevice)
{
    if (!pru_dshot_lib_IsRunning(pwmssDevice))
    {
        (*CM_PER_PWMCSS_CLKCTRL[pwmssDevice]) = 2; // enable module
    }
    return 1;
}
uint8_t pru_dshot_lib_Stop(uint8_t pwmssDevice)
{
    if (pru_dshot_lib_IsRunning(pwmssDevice))
    {
...
uint8_t pru_dshot_lib_ExecCmd(unsigned char* cmd, uint8_t numBytes)
{
    if (cmd[0] == PRU_EDMA_PWMSS_LIB_CMD_ID)
    {
        ...
        {
            pru_dshot_lib_cmd_rsp[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
            pru_dshot_lib_cmd_rsp[1] = PRU_EDMA_PWMSS_LIB_CMD_START_RSP;
            if (numBytes >= 3)
            {
                uint8_t i = 0;
                pru_dshot_lib_cmd_rsp[2] = cmd[2]; // pwmssDevices to start
                pru_dshot_lib_cmd_rsp[3] = 1;
                for (i = 0; i < 3; i++)
                {
                    if (cmd[2] & (1 << i))
                    {
                        pru_dshot_lib_cmd_rsp[3] &= pru_dshot_lib_Start(i); // result
                    }
                }
                return (pru_dshot_lib_config->onStart)(pru_dshot_lib_cmd_rsp, 4)
                        && pru_dshot_lib_cmd_rsp[3];
            }
            else
            {
                pru_dshot_lib_cmd_rsp[2] = 0xFF; // error
                pru_dshot_lib_cmd_rsp[3] = 0; // result KO
                return (pru_dshot_lib_config->onStart)(pru_dshot_lib_cmd_rsp, 4)
                        && pru_dshot_lib_cmd_rsp[3];
            }
        }
        ...
        {
            pru_dshot_lib_cmd_rsp[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
            pru_dshot_lib_cmd_rsp[1] = PRU_EDMA_PWMSS_LIB_CMD_STOP_RSP;
            if (numBytes >= 3)
            {
                uint8_t i = 0;
                pru_dshot_lib_cmd_rsp[2] = cmd[2]; // pwmssDevices to stop
                pru_dshot_lib_cmd_rsp[3] = 1;
                for (i = 0; i < 3; i++)
                {
                    if (cmd[2] & (1 << i))
                    {
                        pru_dshot_lib_cmd_rsp[3] &= pru_dshot_lib_Stop(i); // result
                    }
                }
                return (pru_dshot_lib_config->onStop)(pru_dshot_lib_cmd_rsp, 4)
                        && pru_dshot_lib_cmd_rsp[3];
            }
            else
            {
                pru_dshot_lib_cmd_rsp[2] = 0xFF; // error
                pru_dshot_lib_cmd_rsp[3] = 0; // KO
                return (pru_dshot_lib_config->onStop)(pru_dshot_lib_cmd_rsp, 4)
                        && pru_dshot_lib_cmd_rsp[3];
            }
        }
        ...
        {
            pru_dshot_lib_cmd_rsp[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
            pru_dshot_lib_cmd_rsp[1] = PRU_EDMA_PWMSS_LIB_CMD_SET_DATA_RSP;
            pru_dshot_lib_cmd_rsp[2] = 0;
            pru_dshot_lib_cmd_rsp[3] = 1;
            if (numBytes >= 9)
            {
                /*
                ...
                for (i = 0; i < groups; i++)
                {
                    ...
                    pru_dshot_lib_cmd_rsp[2] |= (1 << numDev);
                    pru_dshot_lib_cmd_rsp[3] &= pru_dshot_lib_SetData(numDev,
                                                                      data[0],
                                                                      data[1],
                                                                      data[2]); // result
                }
                return (pru_dshot_lib_config->onSetData)(pru_dshot_lib_cmd_rsp,
                                                         4)
                        && pru_dshot_lib_cmd_rsp[3];
            }
            else
        ...
        {
            pru_dshot_lib_cmd_rsp[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
            pru_dshot_lib_cmd_rsp[1] = PRU_EDMA_PWMSS_LIB_CMD_SET_DUTY_RSP;
            pru_dshot_lib_cmd_rsp[2] = 0; //
            pru_dshot_lib_cmd_rsp[3] = 1;
            if (numBytes >= 7)
            {
                ...
                for (i = 0; i < groups; i++)
                {
                    ...
                    pru_dshot_lib_cmd_rsp[2] |= (1 << numDev);
                    pru_dshot_lib_cmd_rsp[3] &= pru_dshot_lib_SetDuty(numDev,
                                                                      data[1],
                                                                      data[2]); // result
                }
                return (pru_dshot_lib_config->onSetDuty)(pru_dshot_lib_cmd_rsp,
                                                         4)
                        && pru_dshot_lib_cmd_rsp[3];
            }
            else
        ...
        case (PRU_EDMA_PWMSS_LIB_CMD_SET_PERIOD):
        {
            if (numBytes >= 5)
            {
                 ...
                 */
                pru_dshot_lib_cmd_rsp[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
                pru_dshot_lib_cmd_rsp[1] = PRU_EDMA_PWMSS_LIB_CMD_SET_PERIOD_RSP;
                pru_dshot_lib_cmd_rsp[2] = 0; //
                pru_dshot_lib_cmd_rsp[3] = 1;
                ...
                for (i = 0; i < groups; i++)
                {
                    ...
                    pru_dshot_lib_cmd_rsp[2] |= (1 << numDev);
                    pru_dshot_lib_cmd_rsp[3] &= pru_dshot_lib_SetPeriod(
                            cmd[2], data[1]); // result
                }
                return (pru_dshot_lib_config->onSetPeriod)(
                        pru_dshot_lib_cmd_rsp, 4) && pru_dshot_lib_cmd_rsp[3];
            }
            else
            {
                pru_dshot_lib_cmd_rsp[2] = 0xFF; // error
...
uint8_t pru_dshot_lib_Pulse(uint8_t pwmssDevice)
{
//    if (pru_dshot_lib_config != 0) // se è configurato
//    {
//        if (pru_dshot_lib_initialized[pwmssDevice] == 1) // se è inizializzato
//        {
//            if(!((*CM_PER_PWMCSS_CLKCTRL[pwmssDevice]) & 0x0002)) { // pwmss non è attivo
//                PRU_PWMSS[pwmssDevice]->EPWM_ETCLR = 1; // azzero le interrupt
//                indexFrame[pwmssDevice] = 0;
//                PRU_PWMSS[pwmssDevice]->EPWM_CMPA = FRAME_TO_TRANSFER[indexFrame[pwmssDevice]];
//                PRU_PWMSS[pwmssDevice]->EPWM_CMPB = FRAME_TO_TRANSFER[indexFrame[pwmssDevice]];
//                indexFrame[pwmssDevice]++;
//                pru_dshot_lib_Start(pwmssDevice);
//            } else
            ...
            if (((*CM_PER_PWMCSS_CLKCTRL[pwmssDevice]) & 0x0002)) // se è attivo ...
            {
                ...
                { // finito il periodo carico
                    ...
                    PRU_PWMSS[pwmssDevice]->EPWM_CMPB =
                            FRAME_TO_TRANSFER[indexFrame[pwmssDevice]];
                    indexFrame[pwmssDevice]++;
                    indexFrame[pwmssDevice] = (indexFrame[pwmssDevice] == NUM_FRAME_BLOCK) ? 0 : indexFrame[pwmssDevice];
//                    if (indexFrame[pwmssDevice] == 0)
//                    {
//                        // dovrei caricare il nuovo frame se è stato inviato
//                        // e risettare CMPA/B
//                    }
                }
            }
...
uint8_t pru_dshot_lib_Conf(PruDShotLibConfig* config)
{
    pru_dshot_lib_config = config;
    return 1;
}
uint8_t pru_dshot_lib_SetData(uint8_t pwmssDevice, uint16_t period,
...
uint8_t pru_dshot_lib_SetDuty(uint8_t pwmssDevice, uint16_t duA, uint16_t duB)
{
    PRU_PWMSS[pwmssDevice]->EPWM_CMPA = duA;
    PRU_PWMSS[pwmssDevice]->EPWM_CMPB = duB;
    return 1;
}
uint8_t pru_dshot_lib_SetPeriod(uint8_t pwmssDevice, uint16_t period)
{
    PRU_PWMSS[pwmssDevice]->EPWM_TBPRD = period;
    return 1;
}
